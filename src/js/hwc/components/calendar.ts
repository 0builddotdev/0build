...
import { validateDate } from '$hwc/helpers/date';
...
export class Calendar extends BaseCalendarMixin(InputMixin(Base)) {
  ...
  protected 'input-event' = 'z-calendar:change';
  ...
  protected $cls: Cls = {
    'host-inner': 'z-cal',
    header: 'z-cal-header',
    'previous-button': 'z-button z-button-secondary z-button-icon z-button-small',
    'next-button': 'z-button z-button-secondary z-button-icon z-button-small',
    title: 'z-cal-title',
    jumper: 'z-cal-jumper',
    'month-select': 'z-select z-form-small',
    'year-input': 'z-input z-form-small',
    grid: '',
    weekdays: '',
    weekday: '',
    week: '',
    day: '',
    'day-button': '',
    'jumper-month': 'z-cal-jumper-month',
    'jumper-year': 'z-cal-jumper-year',
    'jumper-button': 'z-button z-button-secondary z-button-icon z-button-small',
    'day-outside-month': 'z-cal-oom',
    'day-selected': 'z-active',
    'day-today': 'z-nothing',
    'day-marked': 'z-cal-marked',
    'button-outside-month': 'z-nothing',
    'button-selected': 'z-nothing',
    'button-today': 'z-nothing',
    'button-marked': 'z-nothing',
  };
  ...
  protected initializeValue(): void {
    if (this.value) {
      try {
        ...
        console.error(`[z-calendar] Invalid date format for value: "${this.value}".`, e);
      }
    } else if (this.today) {
  ...
  private navigate = (event: KeyboardEvent): void => {
    ...
    const buttons = Array.from(this.querySelectorAll<HTMLButtonElement>('button[data-iso]'));
  ...
  private getNextEnabledInColumn(
    ...
    for (let i = startRow; i >= 0 && i < rows.length; i += increment) {
      const button =
        rows[i]?.children[colIndex]?.querySelector<HTMLButtonElement>('button[data-iso]');
  ...
  private getRowFirstEnabledButton(rowIndex: number): HTMLButtonElement | undefined {
    const row = this.querySelectorAll('tr')[rowIndex];

    return Array.from(row?.querySelectorAll<HTMLButtonElement>('button[data-iso]') || []).find(
      b => !b.disabled,
    );
  }

  private getRowLastEnabledButton(rowIndex: number): HTMLButtonElement | undefined {
    const row = this.querySelectorAll('tr')[rowIndex];

    return Array.from(row?.querySelectorAll<HTMLButtonElement>('button[data-iso]') || [])
      .reverse()
      .find(b => !b.disabled);
  }
  ...
  private navigateYear(direction: 'prev' | 'next') {
    ...
    const newYear = direction === 'prev' ? parseInt(year) - 1 : parseInt(year) + 1;
  ...
  private get calendar(): Day[][] {
    ...
    const startingDay = (firstDayOfMonth.getUTCDay() - this['starts-with'] + 7) % 7;
    ...
    for (let week = 0; week < 6; week++) {
      ...
      for (let dayOfWeek = 0; dayOfWeek < 7; dayOfWeek++) {
        ...
        weekDays.push({
          ...
          isDisabled: disabledDatesSet.has(dateStr) || !this.isDateInRange(ISOString),
    ...
    return html`
      ...
      >
        ...
        <table class=${this.$cls['grid']} style=${this.$stl['grid']} role="grid">
          <thead>
            <tr class=${this.$cls['weekdays']} style=${this.$stl['weekdays']} role="row">
              ${repeat(
                this.weekdays,
                day => day,
                day =>
  ...
  private renderHeader() {
    ...
    return html`
      <div data-part="header" class=${this.$cls['header']} style=${this.$stl['header']}>
        ${this.jumpable
          ? nothing
          : html`
              <button
        ...
        <div data-part="title" class=${this.$cls['title']} style=${this.$stl['title']}>
          ${this.jumpable ? this.renderJumper() : html`<span>${monthName} ${year}</span>`}
        </div>
  ...
  private renderJumper() {
    ...
    return html`
      <div data-part="jumper" class=${this.$cls['jumper']} style=${this.$stl['jumper']}>
        ...
        >
          ...
          <select
            ...
            @change=${(e: Event) => this.selectMonth(Number((e.target as HTMLSelectElement).value))}
          >
            ${months.map((m, i) => html`<option value=${i}>${m.substring(0, 3)}</option>`)}
          </select>
          <button
            data-part="jumper-button"
            class="${this.$cls['jumper-button']}"
  ...
  private renderWeek(days: Day[]) {
    return html`
      <tr data-part="week" class=${this.$cls['week']} style=${this.$stl['week']} role="row">
        ${repeat(
          days,
          day => day.ISOString,
